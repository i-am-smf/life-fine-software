...
class database:
    ...
    def connect_database(self):
        self.lfmain_db=sqlite3.connect('database/lifefine_main.db')
        self.lfmain_cursor = self.lfmain_db.cursor()

        self.lfcustomers_db=sqlite3.connect('database/lifefine_customers.db')
        self.lfcustomers_cursor = self.lfmain_db.cursor()

    def create_main_tables(self):

        self.lfmain_cursor.execute("CREATE TABLE IF NOT EXISTS all_customers(id_number VARCHAR(225),\
        ...
        self.lfmain_cursor.execute("CREATE TABLE IF NOT EXISTS compliants(id_number VARCHAR(225),\
            compliant_date VARCHAR(225),\
            compliant_details VARCHAR(225),\
            notes VARCHAR(225))")

    def create_new_user(self):
        self.lfcustomers_cursor.execute(f"CREATE TABLE IF NOT EXISTS `{id}`(visit_date VARCHAR(225),\
    ...
    def insert(self):
        ...
        self.lfmain_cursor.execute(sql_command,values)
        id=id_num.get()
        self.lfmain_db.commit()
...
def list_customer():
    ...
    list_customer_query.iconbitmap("./images/lf2.ico")
